      ...
      <View className="flex-grow flex-row justify-between items-stretch h-[22vh]">
        ...
        <View className="flex flex-col py-2 ml-3 justify-start bg-transparent ring-1 ring-slate-500 h-[23vh] w-[28vw] rounded-lg">
          ...
          <View className="flex flex-row items-center">
            <Text className="text-[10px] text-white font-psemibold text-start text-bold mt-1 " numberOfLines={1}
              ellipsizeMode="tail">
              {product.title}
            </Text>
          </View>
          ...
          {isCounterVisibleMap[index] ? (
            <Counter onClose={() => toggleCounterVisibility(index)} product={product} />
          ) : (
            <TouchableOpacity
              onPress={() => {
